...
The changelog entries below for v3.0 and v4.1 still describe those drop-in directories and are left as they are — they are the record of what those versions shipped, not of what 6.0.0-rc-1 does. Neither mechanism works in 6.0.0-rc-1.
...
Do read the **Important changes in 6.0.0-rc-1** section above first. Your existing posts keep working, but there are a few things worth knowing before you open an old post in the block editor.
...
* NEW: A **Font** setting, under the theme. Fifteen monospaced fonts are offered: Azeret Mono, Cascadia Code, Fira Code, Fira Mono, Google Sans Code, IBM Plex Mono, Inconsolata, JetBrains Mono, M PLUS Code Latin, Nova Mono, Roboto Mono, Source Code Pro, Space Mono, Ubuntu Mono and Victor Mono. The dropdown is split into **With Ligature** and **Without Ligature** with **None** above both, because that is the one question worth asking about a font you are going to read code in: four of them — Cascadia Code, Fira Code, JetBrains Mono and Victor Mono — draw `=>`, `!==` and `&&` as single glyphs, and the preview shows you what that looks like before you choose. **None is the default**, and it is the only choice that costs your visitors nothing: any other loads the font from [Bunny Fonts](https://fonts.bunny.net/), so each reader's browser makes one request to `fonts.bunny.net`. Bunny sets no cookies, keeps no logs and needs no account, but it is still a server that is not yours, so the plugin never calls it unless you ask it to. With None, code boxes keep the font your theme or your own CSS gives them, exactly as before. The chosen font is used in the block editor too, for the code inside the block as you type it, without the ligatures — in an editing box a merged glyph reads as though a character has gone missing.
* CHANGED: Settings which cannot work on their own now move with the setting they need. The copy button lives in the toolbar, and the bracket colours are painted on the brackets the matching finds — so switching either on switches on what it needs, and switching the toolbar or the matching off switches off whatever depended on it. Switching a dependent one off leaves the other alone: a toolbar with no copy button is a perfectly ordinary thing to want.
...
= 6.0.0-rc-1 =
GeSHi is replaced by Prism.js and Gutenberg support added. Requires PHP 8.4 and WordPress 6.9. Old posts keep working, but opening one in the block editor converts its snippets to blocks — read "Important changes in 6.0.0-rc-1" before updating.
