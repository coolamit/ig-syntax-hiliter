...
* CHANGED: Where the `file` label appears now depends on the **Show Toolbar?** setting. With the toolbar on, the label sits in the toolbar — which is Prism's, positioned over the box and faded in on hover or keyboard focus, rather than v5's, which was drawn into the box and always on screen. With the toolbar off, the label is painted in the top right corner of the code box itself and is always visible. That second case is new: in v5 the file label lived only in the toolbar, so turning the toolbar off hid the label too. `file` is an attribute the author asked for, and the toolbar setting is not about it.
