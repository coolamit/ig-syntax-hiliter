...
* NEW: A live preview on the settings page. A sample code box sits beside the settings and repaints as they are changed, so picking one of 43 themes no longer means saving it, opening the front end and coming back. The theme, the font, the toolbar, the copy button and the line numbers all show in it, and it follows each of them as it is switched. Nothing in the preview is saved — it shows what the controls beside it are showing.
