...
* REMOVED: Adding languages by dropping GeSHi language files into a `geshi/` directory is no longer supported, because GeSHi is gone. That covers **both** forms of it — the plugin's own `geshi/` directory (added in v3.0, and still described in the v3.0 entry below as it shipped then) and the `geshi/` directory in a theme (added in v4.1). The plugin no longer ships a `geshi/` directory and no longer looks for one in a theme. Snippets using such a tag are no longer recognised and will show up as ordinary post text with the `[tag]` markers visible. Close to 300 languages ship with the plugin now, so there is almost certainly one for what you were adding; if the tag itself needs to keep working, use the `ig_syntax_hiliter/shortcode_tags` filter.
* CHANGED: The toolbar above a code box shows the language name again, as v5's did. It is Prism's own `show-language` plugin, which carries a display name for every one of the bundled languages, so `cpp` reads **C++**, `csharp` reads **C#** and `markup` reads **Markup**. It appears on hover beside the copy button, and it needs the **Show Toolbar?** setting on.
* CHANGED: The `file` label is written into the markup in full, and exactly as it was typed. v5 did two things to it that v6 does not: it truncated it to 30 characters with an ellipsis on the server, and it ran it through `wp_strip_all_tags()`, which deleted the type parameter out of a name like `vector<int>.cpp`, `Foo<T>.cs` or `List<String>.java`, and everything following an unbalanced `<`. v6 collapses runs of whitespace and leaves the rest alone — the label is never treated as markup anywhere it is used, so the stripping was protecting nothing — then emits the whole path and leaves it to a CSS `max-width` to trim what is shown. The full path is therefore in the page source even when it is displayed shortened.
...
* CHANGED: Settings. `Use plugin CSS for styling?` becomes a **theme** dropdown listing the bundled Prism themes, defaulting to **Okaidia** (with `none` for people who style code boxes themselves). `Show Plain Text Option?` becomes **copy code to clipboard**. `GeSHi Strict Mode?`, `Languages where GeSHi strict mode is disabled` and `Link keywords/function names to Manual?` are gone, as all three were GeSHi features. A new **Limit the height of Gist embeds** option ships **switched on**. `Show Toolbar?`, `Show line numbers in code?`, `Hilite code in comments?` and `Enable GitHub Gist embed in comments?` carry over unchanged, and your existing values are migrated on update.
* BUGFIX: Pasting a post's HTML source into the block editor no longer puts a `<br />` on the end of every line of every snippet in it. WordPress runs a plain text paste through a markdown converter before it looks for shortcodes in it, and that converter turns every newline into a `<br>`, every blank line into a paragraph break and `&`, `<` and `>` into entities. The plugin now undoes exactly those changes on the way into the block, so a pasted snippet holds the code that was on the clipboard. Your own `<br />`, `<br/>` and `<BR>` tags are left alone; a bare lower case `<br>` inside a pasted snippet becomes a newline, because it is byte for byte what the converter itself inserts.
