...
* NEW: A **Before you deactivate** section on the settings page, whose first tool converts this plugin's blocks back to `[sourcecode language="…"]` shortcodes across the whole site. Converted snippets are **not** visible if the plugin is deactivated — an unregistered dynamic block renders as nothing, whereas a shortcode at least remains on screen as text — so this is the way back out. The tool always writes the `[sourcecode]` form, which is semantically identical to the original `[php]`-style tag but is not a byte-for-byte round trip to what was originally typed. Every block converts, this plugin's own tags in the code included: those are written with doubled brackets, which is how a snippet says one of them is text — see the entry on escaped tags below.
* NEW: A block for embedding a GitHub Gist, titled **iG:Syntax Hiliter Gist**. It renders through the same pipeline `[github]` has always used, so the id handling, the link-instead-of-a-script in an excerpt and the **Enable GitHub Gist embed in comments?** setting all apply to it unchanged. Existing `[github]` shortcodes are not converted and go on working exactly as they are; pasting one into the editor offers the block.
