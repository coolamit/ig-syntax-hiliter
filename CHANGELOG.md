...
* NEW: A **Font** setting, below the theme. Ten monospaced fonts are offered: Azeret Mono, Fira Code, Fira Mono, Google Sans Code, JetBrains Mono, M PLUS Code Latin, Nova Mono, Roboto Mono, Source Code Pro and Ubuntu Mono. Azeret Mono, Fira Code and JetBrains Mono carry code ligatures, so `=>`, `!==` and `&&` are drawn as single glyphs; the preview shows which do and which do not. **None is the default and is the only choice which makes no external request:** every other font is loaded from [Bunny Fonts](https://fonts.bunny.net/), meaning one request to `fonts.bunny.net` from each reader's browser. Bunny sets no cookies, stores no logs and needs no account, but it is a third party all the same, so nothing is fetched unless a site owner asks for it. With None, code boxes keep the font they have always had. The chosen font is used in the block editor as well, for the code inside the block while it is being edited, but without ligatures: in an editing box a glyph standing for two characters reads as though one has gone missing.
