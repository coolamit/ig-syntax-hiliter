...
* NEW: 35 more themes for the code boxes, from the [prism-themes](https://github.com/PrismJS/prism-themes) collection (MIT licensed — see `assets/lib/prism-themes/LICENSE`). The **theme** dropdown now offers 43 in all: Prism's own eight, plus One Dark, One Light, Nord, Dracula, VS Code Dark+, Darcula, Gruvbox, Material, Night Owl, Synthwave '84 and the rest. They are listed by name, with **None** at the top of the list rather than the bottom. Exactly one stylesheet is loaded on a page that has a code box, whichever theme is chosen, so the longer list costs a visitor nothing. One theme of that collection is deliberately not bundled: Hopscotch fetches a font from Google, which would have every page carrying a code box call another server.
* NEW: Two options for brackets, using Prism's own `match-braces` plugin. **Point out matching brackets** draws a thin outline around a bracket and its partner when a reader hovers over one, and keeps the pair outlined when they click it; it is **on by default**, because nothing shows until a reader hovers and a page at rest is unchanged. **Colour brackets by depth** gives each level of nesting its own colour and is **off by default**, because it repaints every code box on the site the moment it is switched on. Four of the bundled themes — One Dark, One Light, Coldark Cold and Coldark Dark — carry colours of their own for it and use them; everywhere else the colours are the ones the plugin ships. Neither the script nor its stylesheet is loaded on a page unless one of the two options is on, and the preview beside the settings shows both live.
* NEW: A live preview on the settings page. A sample code box sits beside the settings and repaints as they are changed, so picking one of 43 themes no longer means saving it, opening the front end and coming back. The theme, the font, the toolbar, the copy button, the line numbers and both bracket options all show in it, and it follows each of them as it is switched. Nothing in the preview is saved — it shows what the controls beside it are showing.
* NEW: A **Font** setting, below the theme. Fifteen monospaced fonts are offered: Azeret Mono, Cascadia Code, Fira Code, Fira Mono, Google Sans Code, IBM Plex Mono, Inconsolata, JetBrains Mono, M PLUS Code Latin, Nova Mono, Roboto Mono, Source Code Pro, Space Mono, Ubuntu Mono and Victor Mono. The dropdown is grouped into **With Ligature** and **Without Ligature**, with **None** above both. Four carry code ligatures — Cascadia Code, Fira Code, JetBrains Mono and Victor Mono — so `=>`, `!==` and `&&` are drawn as single glyphs; the preview shows which do and which do not. Every family was measured out of the font file the service actually serves rather than taken from a catalogue, which is how Google Sans Code ended up in the second group despite its name. **None is the default and is the only choice which makes no external request:** every other font is loaded from [Bunny Fonts](https://fonts.bunny.net/), meaning one request to `fonts.bunny.net` from each reader's browser. Bunny sets no cookies, stores no logs and needs no account, but it is a third party all the same, so nothing is fetched unless a site owner asks for it. With None, code boxes keep the font they have always had. The chosen font is used in the block editor as well, for the code inside the block while it is being edited, but without ligatures: in an editing box a glyph standing for two characters reads as though one has gone missing.
* CHANGED: Settings which cannot work on their own now move with the setting they need. **Show the copy button** is drawn inside the toolbar and **Colour brackets by depth** paints the brackets that **Point out matching brackets** finds, so switching either of them on switches on what it needs, and switching a toolbar or the bracket matching off switches off what depended on it. Both toggles move on screen and one message says what happened. The reverse is deliberately not done: switching the copy button off leaves the toolbar alone, because wanting a toolbar without a copy button is a perfectly ordinary thing to want.
...
* CHANGED: The `file` label sits **above** the code box, at the left, and is always visible — whatever the **Show Toolbar?** setting says. In v5 it lived in the toolbar, so turning the toolbar off hid the label as well; `file` is an attribute the author asked for and the toolbar setting is not about it. A snippet given no `file` label gets no label element at all, but it still gets the wrapper — see the markup change below.
...
* BUGFIX: Language-only tags such as `[php]` and `[css]` did not render on many sites running v5.1, as several people reported. They work again.
...
* CHANGED: Every code box is wrapped in `<div class="igsh-code-box">`, and that element carries the box's `id`. Previously the wrapper appeared only when a `file` label was given and the `id` sat on the `<pre>`. If your theme or your own CSS selects a code box as a direct child — `.entry-content > pre`, say — it needs a `>` fewer, or a descendant selector. Selectors on `pre[class*="language-"]` are unaffected, and so is everything Prism does: the `<pre>` keeps its classes and all of its `data-` attributes.
